...
local get_color = require("cincinperin.misc.util").get_color
...
local function configure_hl_groups()
  ...
  vim.api.nvim_set_hl(0, "StatuslineDefault", {
    fg = get_color("Statusline", "fg"),
    bg = statusline_bg,
  })

  vim.api.nvim_set_hl(0, "StatuslineDefaultBold", {
    fg = get_color("Statusline", "fg"),
    bg = statusline_bg,
    bold = true,
  })

  vim.api.nvim_set_hl(0, "StatuslineFaded", {
    fg = get_color("Dimmed", "fg") or get_color("Comment", "fg"),
    bg = statusline_bg,
  })

  vim.api.nvim_set_hl(0, "StatuslinePopout", {
    fg = get_color("WarningMsg", "fg"),
    bg = statusline_bg,
  })

  vim.api.nvim_set_hl(0, "StatuslineSalient", {
  ...
  local mode_fg = statusline_default_bg or (vim.o.background == "dark" and "#000000" or "#ffffff")
  ...
  vim.api.nvim_set_hl(0, "StatuslineFolder", {
    fg = get_color("Normal", "fg"),
    bg = folder_bg,
    bold = true,
  })

  vim.api.nvim_set_hl(0, "StatuslineFolderCap", {
    fg = folder_bg,
    bg = statusline_bg,
  })
  ...
  for _, hl_name in ipairs({
    ...
    vim.api.nvim_set_hl(0, hl_name .. "Cap", {
  ...
  vim.api.nvim_set_hl(0, "StatuslineMulticursor", {
    fg = mode_fg,
    bg = get_color("Special", "fg"),
    bold = true,
  })

  vim.api.nvim_set_hl(0, "StatuslineMulticursorCap", {
    fg = get_color("StatuslineMulticursor", "bg"),
    bg = statusline_bg,
  })
end
...
-- Icon for the multicursor badge: nf-fa-i_cursor (U+F246).
...
local function statusline__mode()
  ...
  return "%#" .. mode_info.hl .. "# " .. center_string(mode_str, mode_name_width) .. " " ..
    "%#" .. cap_hl .. "#" .. mode_sep_r
end
...
local function statusline__folder()
  ...
  return "%#StatuslineFolder#   " .. folder .. " " ..
...
local function statusline__macro_recording()
  ...
  return "%#StatuslinePopout#\243\176\145\138 " .. reg_recording .. " "
end
...
-- Multicursor indicator: a pill badge with the number of active cursors from
-- `multicursors.nvim`. Unlike ronisbr's native-multicursor version, there is no
-- "follow mode" concept for this plugin, so the badge has only one color variant.
local function statusline__multicursor()
  local count = require("cincinperin.misc.multicursor").cursors()
  ...
  return "%#StatuslineMulticursorCap#" .. pill_cap_l ..
    "%#StatuslineMulticursor#" .. text ..
    "%#StatuslineMulticursorCap#" .. pill_cap_r ..
    statusline__space()
end
...
function M.setup()
  ...
  vim.go.statusline = "%{%v:lua.require('cincinperin.misc.statusline').render()%}"
